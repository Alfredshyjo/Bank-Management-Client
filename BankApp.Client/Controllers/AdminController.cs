    ...
    public class AdminController : Controller
    {
        ...
        public async Task<IActionResult> CreateEmployee(EmployeeViewModel model)
        {
            ...
            try
            {
                ...
                // Only reach here if status is 200-299
                if (result.IsError)
                {
            ...
            {
                // This catches validation errors and other API errors
                ModelState.AddModelError(string.Empty, $"Failed to create employee: {ex.Message}");
                return View(model);
            }
            catch (Exception ex)
            {
                ModelState.AddModelError(string.Empty, "An unexpected error occurred while creating the employee.");
                return View(model);
            }
        }
